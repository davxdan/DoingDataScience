...
#spurs_Raw<-xmlTreeParse(siteurl,isHTML = TRUE,useInternalNodes = FALSE)
#class(spurs_Raw)
spurs_html_Raw<-htmlParse(siteurl)
```
```{R}
#spurs_html_tree_Raw<-htmlTreeParse(siteurl)
...
```{r}
separate(spurs_Content, PLAYER, c("PlayerName", "Position"), sep = ",", remove = TRUE, convert = FALSE, extra = "warn", fill = "warn")
spurs_Content
```
